...
class Persyaratan extends CI_Controller {
	...
	{
		$data['dataPersyaratan'] = $this->M_persyaratan->select_all();
		$this->load->view('persyaratan/list_data', $data);
	}
	...
	public function prosesTambah() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$result = $this->M_persyaratan->insert($data);
	...
	public function update($id) {
		...
		$id= $id;
		$data['dataPersyaratan']= $this->M_persyaratan->select_by_id($id);
		$data['content'] = 'persyaratan/form_update';
		$this->load->view('indexadmin',$data);
	...
	public function prosesUpdate(){
		...
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_persyaratan->update($data);
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = '<p class="box-msg">
				      <div class="info-box alert-success">
		...
		$this->session->set_flashdata('msg', $out['msg']);
		redirect('Persyaratan/update/'.$data['id_persyaratan']);
	}

	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_persyaratan->delete($id);
