...
class AboutUs extends CI_Controller {
	...
	{
		$data['dataAboutUs'] = $this->M_aboutus->select_all();
		$this->load->view('aboutus/list_data', $data);
	}
	...
	public function prosesTambah() {
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$result = $this->M_aboutus->insert($data);
	...
	public function update($id) {
		...
		$id= $id;
		$data['dataAboutUs']= $this->M_aboutus->select_by_id($id);
		$data['content'] = 'aboutus/form_update';
		$this->load->view('indexadmin',$data);
	...
	public function prosesUpdate(){
		...
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_aboutus->update($data);
			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = '<p class="box-msg">
				      <div class="info-box alert-success">
		...
		$this->session->set_flashdata('msg', $out['msg']);
		redirect('AboutUs/update/'.$data['id_about']);
	}

	public function delete() {
		$id = $_POST['id'];
		$result = $this->M_aboutus->delete($id);
